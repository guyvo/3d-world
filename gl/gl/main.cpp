...
//40 fps
const int timeRedraw    = 25; //ms

#define SCENE_SPREAD 100
...
void CalcVertexes ( int number );
void CalcColors ( int number );
...
void CBRender (void){
    ...
    glPointParameterf(GL_POINT_SIZE_MAX, 20);
    ...
    glColorPointer(3, GL_FLOAT, 0, (void*)(sizeof(points)));
...
GLfloat randomFloat() {
	return (GLfloat)rand() / ((GLfloat)RAND_MAX + 1.0);
}
...
void CalcVertexes ( int number ){
    for ( int i=0 ; i < number ; i++ ){
        points[i]= (SCENE_SPREAD * randomFloat())-(SCENE_SPREAD/2.0);
    }
...
void * WorkerPoints ( void * ctx ){
   CalcVertexes (sizeof(points)/sizeof(GLfloat));  
   pthread_exit(NULL);
...
void * WorkerColors ( void * ctx ){
   CalcColors(sizeof(colors)/sizeof(GLfloat));
   pthread_exit(NULL);
}
